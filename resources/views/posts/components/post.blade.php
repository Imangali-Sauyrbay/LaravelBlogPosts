<a href="{{ route('posts.show', ['post' => $post->slug]) }}" class="{{ $post->trashed() ? 'text-muted' : '' }}"
    ...
    <div style="display: flex; padding: 10px; margin-bottom: 15px; flex-direction: column; border: 1px solid #ccc">
        ...
        <pre style="margin: 3px; width: 100%; word-break: break-all; white-space: pre-wrap; word-wrap: break-word;"><!--
        -->{{ mb_strcut($post->content, 0, 200) . '...' }}<!--
    --></pre>
