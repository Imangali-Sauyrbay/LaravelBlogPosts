...
class ActivityComposer {
    ...
    {
        $mostCommented = Cache::tags(['blogpost', 'side_bar'])->remember('most_commented_of_all_time',
        now()->addHour(), fn() => Blogpost::mostCommented()->take(5)->get());

        $mostActiveAllTime = Cache::tags(['blogpost', 'side_bar'])->remember('most_active_authors_of_all_time',
        now()->addHour(), fn() => Author::withMostBlogposts()->take(5)->get());

        $mostActiveLastMonth = Cache::tags(['blogpost', 'side_bar'])->remember('most_active_authors_of_last_month',
